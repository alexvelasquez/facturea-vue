...
<div>
    <v-row no-gutters>
        <v-col cols="12">
            <p class="text-lg-h6 font-weight-bold blue-grey--text text--lighten-2">{{title}}</p>
        </v-col>
    </v-row>
    <div>
                    ...
                    <v-expansion-panel-content>
                        <v-row>
                            <v-col cols="12" sm="6" md="6" class="px-5 pl-md-10">
                                <v-row>
                                    <v-col cols="12" md="8">
                                        <v-autocomplete v-model="pedido.cliente" :items="clientes" label="Buscar cliente" item-text="razon_social" item-value="cliente_id" persistent-hint hide-selected dense return-object>
                                    ...
                                    <v-col cols="12" md="4">
                                        <v-menu ref="emision" v-model="menuDate.fechaEmision" :close-on-content-click="false" :return-value.sync="pedido.fecha" transition="scale-transition" offset-y min-width="290px">
                                            <template v-slot:activator="{ on, attrs }">
                                                <v-text-field :rules="reglasValidacion.campoRequerido" v-model="pedido.fecha" label="Fecha" prepend-icon="event" readonly v-bind="attrs" v-on="on" dense outlined persistent-hint></v-text-field>
                                            </template>
                                            <v-date-picker v-model="pedido.fecha" no-title scrollable>
                                                ...
                                                <v-btn text color="primary" @click="$refs.emision.save(pedido.fecha)">OK</v-btn>
                                            </v-date-picker>
                                        </v-menu>
                                    </v-col>
                                        ...
                                        <v-card class="mx-auto" tile>
                                            <v-list-item two-line>
                                                <v-list-item-content>
                                                    ...
                                                    <v-list-item-subtitle>{{pedido.cliente.razon_social}}</v-list-item-subtitle>
                                                </v-list-item-content>
                                            </v-list-item>
                                            ...
                                            <v-list-item two-line>
                                                <v-list-item-content>
                                                    ...
                                                    <v-list-item-subtitle>{{pedido.cliente.documento}}</v-list-item-subtitle>
                                                </v-list-item-content>
                                            </v-list-item>
                            ...
                            <v-col cols="12" sm="6" md="6" class="px-5 pr-md-10" v-if="pedido.cliente">
                                <v-card class="mx-auto" tile>
                                    <v-form ref="formProducto">
                                        <v-list-item two-line>
                                            <v-list-item-content>
                                                ...
                                                <v-autocomplete v-model="nuevoProductoProducto" :rules="reglasValidacion.objetoVacio" :items="productos" label="Buscar" item-text="descripcion" item-value="producto_id" persistent-hint hide-selected return-object :hint="hint">
                                                    <template v-slot:item="data">
                                                        <v-list-item-content>
                                                            <v-list-item-title v-html="data.item.descripcion"></v-list-item-title>
                                                            <v-list-item-subtitle v-html="'Código: '+data.item.codigo"></v-list-item-subtitle>
                                                        </v-list-item-content>
                                                    </template>
                                                </v-autocomplete>
                                            </v-list-item-content>
                                            ...
                                            <v-list-item-content>
                                                ...
                                                <v-row>
                                                    <v-col cols="12" :md="esResponsableInscripto?'3':'4'">
                                                        <v-text-field v-model="nuevoProductoCantidad" :rules="reglasValidacion.cantidad" label="Cantidad" outlined placeholder="0" min="0.1" type="number" dense></v-text-field>
                                                    </v-col>
                                                    <v-col cols="12" :md="esResponsableInscripto?'3':'4'">
                                                        <v-text-field v-model="nuevoProductoBonificacion" color="#385F73" label="Bonificación.(%)" outlined placeholder="0" type="number" dense></v-text-field>
                                                    </v-col>
                                                    <v-col cols="12" :md="esResponsableInscripto?'3':'4'" v-if="esResponsableInscripto">
                                                        <v-select v-model="nuevoProductoIva" persistent-hint :items="alicuotas" :rules="reglasValidacion.objetoVacio" item-text="descripcion" placeholder="0" label="IVA(%)" hide-selected return-object dense outlined></v-select>
                                                    </v-col>
                                                    <v-col cols="12" :md="esResponsableInscripto?'3':'4'">
                                                        <v-text-field v-model="nuevoProductoSubtotal" color="#385F73" label="Subtotal" outlined placeholder="0" type="number" dense disabled></v-text-field>
                                                    </v-col>
                                                </v-row>
                                            </v-list-item-content>
                                        </v-list-item>
                                    ...
                                    <v-list-item two-line :disabled="!nuevoProductoProducto.producto_id">
                                        <v-list-item-content>
                                            <v-btn block color="#385F73" dark @click="agregarPedido()">Agregar</v-btn>
                                        </v-list-item-content>
                                    </v-list-item>
                                </v-card>
                            </v-col>
    ...
    <v-card class="mt-4">
        <v-row v-if="listado.length> 0">
            <v-col cols="12">
                <v-data-table v-model="selected" :headers="headers" :items="listado" item-key="producto.producto_id" show-select hide-default-footer>
                    ...
                    <template v-slot:item.actions="{ item }">
                        <v-icon medium color="#385F73" class="mr-2" v-bind="item" @click="modalItemProducto(item)">
                            mdi-pencil
                        </v-icon>
                        <v-icon medium color="red" class="mr-2" v-bind="item" @click="eliminarItemProducto(item)">
                            mdi-close
                        </v-icon>
                    </template>
                </v-data-table>
                <v-row>
                    ...
                    <modal-pedido :dialog="dialog" :nuevo-producto="nuevoProducto" :hint="hint" :productos="productos" :reglas="reglasValidacion" :tipos-ali-cuotas="alicuotas" @cerrar-dialog="cerrarDialog" @editar-producto="editarProducto($event)">
                    </modal-pedido>
                </v-row>
            </v-col>
        </v-row>
        ...
        <v-row>
            <v-col cols="12" md="" align="right">
                <v-col cols="12" sm="12" md="6">
                    <v-btn color="#385F73" dark block @click="pedidoId?modificarPedido():cargarPedido()">{{pedidoId?'Modificar':'Cargar' }} Pedido</v-btn>
                </v-col>
            </v-col>
            <v-col cols="12" md="4" align="right">
                <v-list-item>
                    <v-list-item-content>
                        ...
                        <v-list-item-title class="headline mb-1">{{pedido.total|formatPrecio}}</v-list-item-title>
                    </v-list-item-content>
                </v-list-item>
            </v-col>
...
export default {
        ...
        cargarDatosPedido() {
                ...
                if (this.pedidoId) {
                    axios.get(`preventas/${this.pedidoId}`)
                        .then(response => {
                            this.pedido.cliente = response.data.data.preventa.cliente;
                            this.pedido.productos = response.data.data.productos;
                ...
                this.$swal({
                    ...
                    if (result.value) {
                        ...
                        if (item.producto_preventa_id) {
                            axios.delete(`productoPreventa/eliminar/${item.producto_preventa_id}`)
                                .then(response => {
            ...
            actualizarStock(item, tipo) {
                let indice = this.productos.map(function(e) {
                    ...
                    case 'editar':
                        if (this.productos[indice].producto_id == item.producto.producto_id) {
                            if (item.cantidad_previa < item.cantidad) {
                                this.productos[indice].stock = this.productos[indice].stock - (item.cantidad - item.cantidad_previa);
                            } else if (item.cantidad_previa > item.cantidad) {
                                this.productos[indice].stock = this.productos[indice].stock + (item.cantidad_previa - item.cantidad);
                            }
            ...
            cargarPedido() {
                this.$swal({
                    ...
                    if (result.value) {
                        axios.post(`preventas/negocio/${this.negocio.negocio_id}/nuevo`, this.pedido)
                            .then(response => {
            ...
            modificarPedido() {
                this.$swal({
                    ...
                    if (result.value) {
                        axios.put(`preventas/preventa/${this.pedidoId}/editar`, this.pedido)
                            .then(response => {
    ...
    watch: {
        nuevoProductoProducto() {
            this.hint = this.nuevoProductoProducto.producto_id ? 'Precio: $' + this.calcularPrecioNeto(this.nuevoProductoProducto) + ' - Disponibles: ' + this.nuevoProductoProducto.stock : '';
        },
    },
    computed: {
        listado: {
        ...
        reglasValidacion() {
            return {
                cantidad: [
                    ...
                    v => v <= this.nuevoProductoProducto.stock || `Debe ser menor o igual al stock disponible:${this.nuevoProductoProducto.stock}`
                ],
                objetoVacio: [
