...
<div>
    <v-row no-gutters>
        <v-col cols="12">
            <p class="text-lg-h6 font-weight-bold blue-grey--text text--lighten-2">Categorias</p>
        </v-col>
    </v-row>
    <v-card>
        ...
        <v-card-title>
            <v-col cols="6" sm="4">
                <v-row>
                    <v-text-field v-model="search" append-icon="mdi-magnify" label="Buscar Categoria" single-line hide-details></v-text-field>
                </v-row>
            </v-col>
            <v-spacer></v-spacer>
        ...
        <v-data-table v-model="seleccionados" :headers="cabeceras" item-key="categoria_id" :items="categorias" :search="search" show-select>
        ...
        <v-row id="actions-productos">
            <v-col cols="12" sm="12" md="12" align="end">
                <v-btn v-if="seleccionados.length > 0" outlined color="#385F73" dark class="mb-2 mr-md-2">ELIMINAR SELECCIONADOS
                </v-btn>
                <modal-aumento :categorias="categorias"></modal-aumento>
                <v-btn outlined color="#385F73" @click="$router.push('/productos')" dark class="mb-2 mr-md-2">IR A PRODUCTOS
                </v-btn>
                <modal-categoria :editable="editable" :dialog="dialog" :categoria="itemCategoria" @cerrar-dialog="close" @agregar-categoria="agregarCategoria($event)" @editar-categoria="editarCategoria($event)">
                </modal-categoria>
                <v-btn outlined color="#385F73" dark class="mb-2 mr-md-2" @click="exportar(`categorias/exportar/${negocio.negocio_id}`,'listado_categorias.pdf')" v-if="categorias.length>0">EXPORTAR
                </v-btn>
            </v-col>
        </v-row>
    </v-card>
        ...
        cargarCategorias() {
            ...
            eliminarCategoria(item) {
                this.$swal({
                    ...
                    if (result.value) {
                        axios.put(`categorias/eliminar/${item.categoria_id}`)
                            .then(response => {
            ...
            eliminarSeleccionados() {
                this.$swal({
                    ...
                    if (result.value) {
                        axios.put(`categorias/eliminarCategorias`, {
                                marcas: JSON.stringify(this.seleccionados)
                            })
                            .then(response => {
                                this.seleccionados.forEach(element => this.categorias.splice(this.categorias.indexOf(element), 1))
