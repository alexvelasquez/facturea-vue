    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-btn
            small
            :color="!tieneFactura(item) ? 'primary' : 'error'"
            class="mr-3"
            dark
            @click="accionFactura(item)"
            rounded
          >
          ...
          <v-btn
            small
            :color="!tienePedido(item) ? 'primary' : 'error'"
            dark
            @click="accionPedido(item)"
            rounded
          >
...
export default {
  ...
  async mounted() {
    this.usuarios = (await usuarios()).data.data;
  },
  methods: {
    async accionPedido(item) {
      const valor = item.negocio.pedido == "S" ? "N" : "S";
      const response = (await editarPedidoUsuario(item.id,valor))
      if(response.status === 200){
            this.notificacion(`Sección pedido ${valor === 'S' ? 'activado' : 'desactivado'}.`, "success");
            item.negocio.pedido = item.negocio.pedido == "S" ? "N" : "S";
      }
      else{
    ...
    accionFactura(item) {
      ...
      axios
        .put(`/usuario/gestionFacturaElectronica/${item.id}`, { valor: valor })
        .then((response) => {
